...
export class WorkflowRecovery {
  constructor(private readonly store: WorkflowStore, private readonly dispatcher: TaskDispatcher, private readonly staleAfterMs = 60_000) {}
  ...
  async recover(): Promise<number> {
    ...
    for (const status of ['queued', 'running'] as const) {
      ...
      for (const execution of executions) {
        if (Date.now() - Date.parse(execution.updatedAt) < this.staleAfterMs) continue;
